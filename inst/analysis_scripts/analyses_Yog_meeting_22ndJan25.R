...
wastewater_shedding_relative_SC2 <- seq(log10(0.001), log10(100), 0.2)
R0_scan <- c(0.50, 0.95)  # c(0.5, 0.75, 0.95, 1.25)
annual_spillover_rate_scan <- 5
prob_symptomatic_scan <- seq(0.05, 1, 0.05)
prob_seek_healthcare_scan <- c(0.1, 0.9)
iterations_scan <- 1:20
...
combined_df_time_detection_wastewater_summary <- combined_df_time_detection_wastewater %>%
  ...
  summarise(wastewater_first_day_median = median(wastewater_first_day, na.rm = TRUE),
            ...
            wastewater_seeking_outbreak_size_lower = quantile(wastewater_outbreak_size, 0.25, na.rm = TRUE),
            wastewater_seeking_outbreak_size_upper = quantile(wastewater_outbreak_size, 0.75, na.rm = TRUE))
...
ggplot(combined_df_time_detection_wastewater_summary,
  ...
  scale_x_log10(breaks = c(1e-2, 1e-1, 1, 10, 100),
                labels = paste0(rev(c(1e-2, 1e-1, 1, 10, 100)), "x")) +
...
ggplot(combined_df_outbreak_wastewater_summary,
       aes(x = threshold, y = wastewater_outbreak_size_median)) +
  geom_line() +
